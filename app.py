...
<body>
    ...
    <section class="min-h-screen flex flex-col justify-center items-center text-center px-6 pt-20">
        <div class="reveal active space-y-6 max-w-4xl">
            ...
            <p class="text-xl md:text-2xl text-gray-600 mt-4 max-w-2xl mx-auto">実験データの解析から、難解な物理法則の理解まで。<br>機械工学科での日々を、劇的に加速させるパートナー。</p>
        </div>
        <div class="mt-16 w-full max-w-5xl scale-reveal">
            <div class="relative aspect-video bg-gradient-to-br from-gray-100 to-gray-200 rounded-3xl shadow-2xl overflow-hidden flex items-center justify-center border border-white">
                <div class="text-center space-y-4"><div class="text-9xl">🤖 ⚡️ ⚙️</div><p class="text-gray-400 font-bold tracking-widest uppercase">Engineering Intelligence</p></div>
                <div class="absolute top-10 left-10 bg-white p-4 rounded-2xl shadow-lg animate-bounce" style="animation-duration: 3s;"><code class="text-sm text-blue-600 font-mono">import numpy as np</code></div>
                <div class="absolute bottom-20 right-10 bg-white p-4 rounded-2xl shadow-lg animate-bounce" style="animation-duration: 4s;"><span class="text-xl font-serif italic">F = ma</span></div>
            </div>
        </div>
    </section>
        ...
        <div class="max-w-5xl mx-auto px-6">
            <div class="grid md:grid-cols-2 gap-16 items-center">
                <div class="reveal">
                    ...
                    <p class="text-lg text-gray-600 leading-relaxed">CSVファイルをアップロードするだけ。「このデータの散布図を描いて」「近似曲線を求めて」と頼めば、Pythonコードごと出力。単位の換算ミスも、次元解析のチェックも、もう心配いりません。</p>
                </div>
                <div class="scale-reveal relative group">
                    ...
                    <div class="relative bg-gray-900 rounded-2xl p-6 shadow-2xl text-white font-mono text-sm overflow-hidden">
                        ...
                        <p class="text-green-400"># User Input</p><p class="mb-4">応力-ひずみ線図を作成し、ヤング率を求めて。</p><p class="text-blue-400"># ChatGPT Output</p><p>import pandas as pd<br>import matplotlib.pyplot as plt<br>...</p>
                        <div class="mt-4 bg-white rounded border border-gray-700 overflow-hidden">{stress_img_tag}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="py-32 bg-[#f5f5f7]">
        <div class="max-w-6xl mx-auto px-6">
            <div class="text-center mb-20 reveal"><h2 class="text-4xl md:text-6xl font-bold">あらゆる課題を、<br>シンプルに解決。</h2></div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 h-auto md:h-[800px]">
                <div class="md:col-span-2 md:row-span-2 bg-white rounded-3xl p-8 shadow-sm hover:shadow-xl transition duration-500 scale-reveal flex flex-col justify-between overflow-hidden relative">
                    <div class="z-10"><h3 class="text-3xl font-bold mb-2">難解な論文も、<br>3行で要約。</h3><p class="text-gray-600 mt-4 max-w-md">英語の論文PDFをアップロード。「この論文の結論と、実験条件の限界を教えて」と聞くだけで、要点を抽出。先行研究の調査時間を大幅に短縮します。</p></div>
                    <div class="absolute bottom-[-50px] right-[-50px] w-80 h-80 bg-blue-100 rounded-full blur-3xl opacity-50"></div>
                    {paper_img_tag}
                </div>
                <div class="bg-white rounded-3xl p-8 shadow-sm hover:shadow-xl transition duration-500 scale-reveal flex flex-col justify-center items-center text-center"><div class="text-5xl mb-4">🔬</div><h3 class="text-xl font-bold">レポート作成支援</h3><p class="text-sm text-gray-500 mt-2">「考察の切り口を提案して」<br>※コピペは厳禁。思考の補助輪として。</p></div>
                <div class="bg-black text-white rounded-3xl p-8 shadow-sm hover:shadow-xl transition duration-500 scale-reveal flex flex-col justify-between"><div><h3 class="text-xl font-bold text-purple-400">Code Assistant</h3><p class="text-sm text-gray-400 mt-2">Arduino, Python, MATLAB.<br>エラーの原因を一瞬で特定。</p></div><div class="mt-4 bg-gray-800 p-3 rounded-lg text-xs font-mono text-green-400">> Error fixed.</div></div>
                <div class="md:col-span-3 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-3xl p-10 text-white shadow-lg scale-reveal flex flex-col md:flex-row items-center justify-between"><div class="mb-6 md:mb-0"><h3 class="text-3xl font-bold">アイデア出しの壁打ち相手。</h3><p class="text-indigo-100 mt-2">設計課題のブレーンストーミングや、プレゼンの構成案まで。</p></div><a href="https://kuriryo2005.github.io/chatgpt-synapse" target="_blank" class="bg-white text-indigo-600 px-8 py-3 rounded-full font-bold hover:bg-opacity-90 transition shadow-lg">プロンプトを見る</a></div>
            </div>
        </div>
    </section>
    <section class="py-32 bg-white text-center">
        <div class="max-w-4xl mx-auto px-6 reveal">
            <p class="text-gray-500 font-medium mb-4">圧倒的な効率化</p><h2 class="text-5xl md:text-7xl font-bold mb-16">もう、<br>単純作業に時間を割かない。</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10 text-center">
                <div><div class="text-6xl font-black text-blue-600 mb-2">50<span class="text-3xl">%</span></div><p class="text-gray-600 font-medium">プログラミング時間の短縮</p></div>
                <div><div class="text-6xl font-black text-blue-600 mb-2">10<span class="text-3xl">x</span></div><p class="text-gray-600 font-medium">アイデアの創出数</p></div>
                <div><div class="text-6xl font-black text-blue-600 mb-2">∞</div><p class="text-gray-600 font-medium">いつでも質問可能なTAとして</p></div>
            </div>
        </div>
    </section>
    <section class="py-20 bg-[#f5f5f7]">
        <div class="max-w-4xl mx-auto px-6 reveal">
            ...
            <div class="flex flex-col md:flex-row justify-center gap-4">
                ...
                <a href="https://kuriryo2005.github.io/chatgpt-f1-guide" target="_blank" class="bg-white text-black border border-gray-300 px-8 py-4 rounded-full font-bold text-lg hover:bg-gray-50 transition shadow-sm">プロンプト集を見る</a>
            </div>
