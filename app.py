...
def get_audio_html(file_name):
    ...
    <div id="music-fab" class="sound-fab" onclick="toggleMusic()">
        <div id="icon-container">{ICON_PLAY}</div>
    </div>
    ...
    <script>
        ...
        function toggleMusic() {{
            if (isPlaying) {{
                ...
                btn.classList.remove("is-active"); // アニメーション停止
                isPlaying = false;
            }} else {{
                audio.volume = 0.5;
                audio.play()
                    .then(() => {{
                        ...
                        btn.classList.add("is-active"); // アニメーション開始
                        isPlaying = true;
                    }})
